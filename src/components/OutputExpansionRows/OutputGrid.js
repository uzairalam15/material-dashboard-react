...
class OutputGrid extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentDidMount() {
    if (this.props.globalFunctionId && !this.props.data.length) {
      this.props.getOutputs(this.props.globalFunctionId);
    }
  }

  onSubmit = data => {
    this.props.createOutputAction(
      Object.assign({}, data, {
  ...
  getOtherDiv = loader => {
    return loader ? (
      ...
      <h3 style={{ textAlign: "left" }}>No Outputs</h3>
    );
  };

  render() {
    const noDataText = <h3 style={{ textAlign: "left" }}>No Outputs</h3>;
    const { classes, data, functions, loader } = this.props;
    return (
