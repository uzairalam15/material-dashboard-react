...
class FailureModeElement extends React.PureComponent {
  ...
  getFailureModeElements = failureModes => {
    ...
    return <h5 style={{ textAlign: "center" }}>No FailureModes</h5>;
  };

  render() {
    const { classes, item, failureModes, functions } = this.props;
    ...
    return (
      <GridItem xs={12} sm={12} md={12}>
        <ExpansionPanel expanded={open} onChange={this.toggleRow}>
            ...
            <CardHeader style={{ width: "100%", margin: 6 }} color="info">
              <Grid container>
                <GridItem xs={12} lg={12} md={12} style={{ paddingLeft: 0 }}>
                  <h4 className={classes.cardTitleWhite}>FailureModes</h4>
                  <p className={classes.cardCategoryWhite}>
                    Expand to failureModes and its dependent elements
                  </p>
                </GridItem>
              </Grid>
            </CardHeader>
          ...
          >
            <CardBody style={{ padding: 0 }}>
              ...
              {/* <Tasks
                items={failureModes}
                toggleEditModal={this.toggleFailureModeModal}
                deleteFailureMode={this.props.deleteFailureModeAction}
              /> */}
            </CardBody>
          </ExpansionPanelDetails>
        ...
        <CreateFailureModeModal
          title={`${this.state.modalMode.toUpperCase()} FailureMode`}
